# Personal Print Licence

**Double Tree Photography - Portrait Session Licence Agreement**

This Personal Print Licence ("Licence") accompanies the photographs delivered from your portrait session with Double Tree ("the Photographer"). By downloading, printing, or otherwise using the delivered images ("the Images"), you ("the Client") agree to the terms below.

---

## 1. Grant of Licence

The Photographer grants the Client a **non-exclusive, non-transferable, worldwide, perpetual licence** to use the Images for **personal, non-commercial purposes only**.

This Licence takes effect once the balance of the session fee has been paid in full. Downloading or using the Images before the balance is paid in full does not grant the Client any rights under this Licence.

## 2. Permitted Uses

Under this Licence, the Client **may**:

- Make **prints** of the Images for personal use, display, and gifting to family and friends.
- Create personal products such as albums, cards, framed wall art, and calendars.
- Post the Images to **personal** social media and messaging accounts, provided they are not altered beyond cropping or rotation and are not used to promote a business.
- Have prints and personal products made by professional or consumer print labs and print services, for the Client's own personal use.
- Store personal backup copies of the delivered files.

Credit to "Double Tree" is appreciated on public posts but is not required for personal use.

## 3. Prohibited Uses

Under this Licence, the Client **may not**:

- Use the Images for any **commercial, promotional, or advertising purpose**, including but not limited to business websites, paid advertising, product packaging, brand or company social media accounts, printed marketing materials, or any use intended to generate revenue or promote goods or services.
- **Sell, license, sublicense, or transfer** the Images (or the right to use them) to any third party, except to engage a print lab or print service as permitted in Section 2.
- Enter the Images into **contests**, or submit them to stock libraries, agencies, or publications.
- Except for cropping or rotation as permitted in Section 2, **alter, edit, filter, composite, or apply AI processing** to the Images in any way that **misrepresents** the Photographer's work or original edit, or use the Images to train machine-learning or generative-AI systems.
- **Remove, crop out, or obscure** any watermark, credit, or metadata where present.

Any use beyond the scope of Section 2 requires a separate **Commercial Licence** (see Section 6).

## 4. Copyright and Ownership

The Photographer **retains full copyright** and ownership of all Images at all times, including all raw, unedited, and edited files, in accordance with the _Copyright Act_ (R.S.C., 1985, c. C-42) of Canada. This Licence grants usage rights only and does **not** transfer copyright or ownership.

## 5. Photographer's Rights

The Photographer retains the right to use the Images for **portfolio, promotional, and marketing purposes**, including the Photographer's website, social media, printed portfolio, and submissions to competitions or publications.

The Photographer may also capture and use **behind-the-scenes media** - including photographs and video of the session, its setup, and the shooting process - for the Photographer's own marketing, portfolio, social media, and educational content. This may incidentally include the Client's likeness. The Client consents to this use unless they opt out in writing before the session. Where the session includes a minor, the parent or guardian who books the session gives and may withdraw this consent on the minor's behalf.

If the Client wishes to keep their session **private**, they must request this in writing before delivery; a privacy request may be accommodated at the Photographer's discretion.

## 6. Commercial Use

Commercial use of the Images is **not** included in the Headshot, Portrait, or Film sessions or this Personal Print Licence. If the Client wishes to use any Image commercially, a separate **Commercial Licence** must be negotiated and agreed in writing, with fees based on the scope, duration, and territory of the intended use. Commercial licensing is available on request.

## 7. Delivery and Availability

Images are delivered via a private, password-protected online gallery. Galleries remain active for **60 days** from the delivery date, after which the Client is responsible for having downloaded and backed up their files. Re-delivery after expiry may be subject to an archival retrieval fee.

## 8. Liability

The Photographer's total liability under this Licence is limited to the amount paid by the Client for the session. The Images are provided for personal enjoyment, and the Photographer is not liable for any loss arising from the Client's use of the Images beyond the terms of this Licence.

## 9. Governing Law and Disputes

This Licence is governed by the laws of the Province of **British Columbia** and the applicable laws of Canada. The Client and the Photographer submit to the exclusive jurisdiction of the courts of British Columbia for any dispute arising from this Licence.

## 10. General

**Severability.** If any provision of this Licence is found unenforceable, the remaining provisions stay in full effect.

**Entire agreement.** This Licence, together with the session booking it accompanies, is the entire agreement between the Client and the Photographer regarding use of the Images.

**Revocation.** The Photographer may revoke this Licence on written notice if the Client materially breaches it - in particular by using the Images commercially without a Commercial Licence (Section 6).

---

_By downloading or printing the delivered Images, the Client acknowledges that they have read, understood, and agreed to the terms of this Personal Print Licence._

**Double Tree Studio Inc.** · Vancouver, BC
